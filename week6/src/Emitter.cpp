...
void Emitter::init()
{
    ...
    m_pVB = wolf::BufferManager::CreateVertexBuffer(vertices, sizeof(Vertex) * NUMPARTICLES * 6);
    ...
    if (blending == Additive)
    {
        ...
        m_pMat->SetBlendMode(wolf::BM_SrcAlpha, wolf::BM_DstAlpha);
    }
    ...
    m_pMat->SetTexture("tex", wolf::TextureManager::CreateTexture(texture));
}
...
void Emitter::update(float dt, const glm::mat4 &viewMatrix)
{
    ...
    m_pVB->Write(vertices, sizeof(Vertex) * 6 * numberOfActiveParticles);
}
